...
{
    public GameObject PlanetObject; // The Jupiter object
    public Vector3 RotationVector = new Vector3(0, 6.25f, 0); // Rotation speed of Jupiter
    public Transform Sun; // Reference to the Sun
    public float orbitalSpeed = 13.07f; // Jupiter's average orbital speed (scaled)
    public float orbitalRadius = 5.20f; // Average distance from the Sun in AU (scaled)
    ...
    private const float JupiterAxialTilt = 3.1f; // Jupiter's axial tilt in degrees
    private float angle; // Current angle of Jupiter in its orbit

    private void Start()
    {
        // Set Jupiter's initial tilt
        ...
        lineRenderer.startColor = Color.white; // Color for Jupiter's orbit
    ...
    private void Update()
    {
        ...
        // Calculate the new position of Jupiter in its orbit
        float x = Mathf.Cos(angle) * orbitalRadius;
        float z = Mathf.Sin(angle) * orbitalRadius;

        // Set the new position of Jupiter
